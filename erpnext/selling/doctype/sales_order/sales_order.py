...
class SalesOrder(SellingController):
	...
	def validate_sales_mntc_quotation(self):
		for d in self.get('items'):
			if d.prevdoc_docname:
				res = frappe.db.sql("select name from `tabOpportunity` where name=%s and opportunity_type = %s",
	...
	def validate_with_previous_doc(self):
		...
		for quotation in list(set([d.prevdoc_docname for d in self.get("items")])):
			if quotation:
				frappe.db.set_value("Opportunity",quotation,"status","Won")
				frappe.db.set_value("Lead",self.lead,"status","Awaiting for AV")
	...
	def update_prevdoc_status(self, flag):
		for quotation in list(set([d.prevdoc_docname for d in self.get("items")])):
			if quotation:
				pass
				#doc = frappe.get_doc("Opportunity", quotation)
				frappe.db.set_value("Opportunity",quotation,"status","Won")
				frappe.db.set_value("Lead",self.lead,"status","Awaiting for AV")
				#if doc.docstatus==2:
				#	frappe.throw(_("Opportunity {0} is cancelled").format(quotation))
